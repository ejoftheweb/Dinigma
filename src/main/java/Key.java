...
private static Marker mark = MarkerManager.getMarker("Key");
...
public Key(File keyFile)throws MinigmaException {
 	try{
	  ...
	  skrc = new PGPSecretKeyRingCollection(instream, kfpc);
          ...
          fileStream.close();
            ...
            log.debug(mark, "problem loading Key from file", x);
    ...
    public Key(InputStream inputStream)throws MinigmaException {
      try{
            ...
            log.debug(mark, "problem loading Key from input stream", x);
 ...
 private void init(){
   try{
      ...
      if (signingKey == null) {
		...
		log.debug(mark,"K-init problemo", e);
	}
}
 /**
  *
...
public PGPSecretKey getDecryptionKey(long keyID){
	...
	return skrc.getSecretKey(keyID);
        ...
        log.debug(mark,"Key: couldn't find decryption key for "+Kidney.toString(keyID));
        return null;
    }
}
